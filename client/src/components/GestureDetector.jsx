...
const GestureDetector = ({onGesture}) => {
 ...
 const MODEL_URL = "https://teachablemachine.withgoogle.com/models/sG3DX3BM_/model.json"
  ...
  const [classifire, setClassfier] = useState(null);
...
loadModel();
  ...
  const classifyGesture = () => {
    if (classifire && videoRef.current){
      classifire.classify(videoRef.current, (results, error) => {
        if(error){
